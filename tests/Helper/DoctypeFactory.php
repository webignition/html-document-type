...
class DoctypeFactory
{
    ...
    /**
     * Create a set of docTypeValues featuring:
     * - varying case for DOCTYPE prefix
     * - varying case for root element
     * - varying case for FPI
     *
    ...
    public static function createSetFromValues(array $sourceDocTypeValues, $key = '')
    {
        ...
        foreach (self::$docTypePrefixes as $prefixKey => $docTypePrefix) {
            $docTypeSetValues['prefix: ' . $prefixKey . ': ' . $key] = array_merge(
                $sourceDocTypeValues,
                [
        ...
        foreach ($rootElementValues as $rootElementKey => $rootElementValue) {
            $docTypeSetValues['root element: ' . $rootElementKey . ': ' . $key] = array_merge(
                $sourceDocTypeValues,
                [
        ...
        foreach ($fpiValues as $fpiKey => $fpiValue) {
            $docTypeSetValues['fpi: ' . $fpiKey . ': ' . $key] = array_merge(
                $sourceDocTypeValues,
                [
